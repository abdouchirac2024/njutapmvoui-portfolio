...
This system handles the daily automatic sends very well. What it didn't yet handle was a manual bulk trigger — an admin who wants to remind 1,000 students *right now*, not tomorrow morning. The answer isn't to process that synchronously inside the HTTP request: it's inserting the 1,000 rows in a single batched transaction (fast), responding immediately, then letting the same scheduled worker — or a batched variant, 20 to 50 at a time — drain the queue at a pace the database and external channels can absorb.
